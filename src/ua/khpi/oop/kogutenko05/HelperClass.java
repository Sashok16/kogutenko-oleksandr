...
public class HelperClass extends Object{
		...
		try {
			while (check) {
				...
				switch (input) {
					case "remove": {
						if(save.size() > 0) {
							System.out.println("Enter some string which can contains at this array and you want to delete\n-> ");
					...
					case "contain": {
						if(save.size() > 0) {
							System.out.println("Enter some string which can contains at this array\n-> ");
							String in = scanner.nextLine();
							if (save.contains(in)) {
	...
	public void changedText() throws Exception {
		...
		{
			System.out.println("Do you want to change some string from array or your string?(my - 0, array - 1)");
